...
Future<void> WriteUserData(Map<String, dynamic> userData) async {
  ...
  await prefs.setString('id', userData['data']['_id']);
...
// Function to read user data from SharedPreferences
Future<String?> ReadUserData(String key) async {
  final prefs = await SharedPreferences.getInstance();
  return prefs.getString(key);
}

// Function to remove the token (clear all shared preferences)
...
Uint8List ShowBase64Image(String base64String) {
  ...
  return data?.contentAsBytes() ?? Uint8List(0); // Return an empty Uint8List if parsing fails
}
